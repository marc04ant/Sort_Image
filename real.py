...
from tkinter import Tk, Label, Button, Canvas, filedialog # tkinter is to create the graphic interface
from PIL import Image, ImageTk, ImageOps # Pillow package used for image processing and display
...
class ImageViewer: # We create this class to handle the main window and allow displaying image
    ...
    def show_current_image(self): # This function display the image at a specific index
        image_path = self.image_paths[self.current_image_index] # We define which image to display
    ...
    def select_image(self): # This function copy the current displayed image into an other folder where we store selected images.
    ...
    def deselect_image(self):
        ...
        image_name = os.path.basename(image_path) # here we define just the name of the folder
        ...
        try:
            ...
            self.show_current_image()
            self.update_deselect_button_state()
        except FileNotFoundError: #if there is not such a file in the storage directory we display this message.
